...
impl<'a> WebauthnBuilder<'a> {
    ...
    pub fn new(rp_id: &'a str, rp_origin: &'a Url) -> WebauthnResult<Self> {
        ...
        let valid = rp_origin
            ...
            // Browsers permit loopback IP origins for local development. The public
            // WebAuthn RP ID rules still require an exact host match in that case.
            .or_else(|| rp_origin.host_str().map(|host| host == rp_id))
            .unwrap_or(false);
