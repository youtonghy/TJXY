import {getFirstItem as $8Gj3P$getFirstItem, getChildNodes as $8Gj3P$getChildNodes} from "react-stately/private/collections/getChildNodes";
import {Rect as $8Gj3P$Rect, Layout as $8Gj3P$Layout} from "react-stately/useVirtualizerState";
...
class $47544ce7688f5862$export$64943d2e59d72a29 extends (0, $8Gj3P$Layout) {
    constructor(options = {}){
    ...
    getKeyBelow(key) {
        ...
        let rect = new (0, $8Gj3P$Rect)(layoutInfo.rect.x, layoutInfo.rect.maxY + 1, layoutInfo.rect.width, this.virtualizer.visibleRect.height);
        let closestRow = this.collection.getItem(this._findClosest(layoutInfo.rect, rect)?.key);
        return (0, $8Gj3P$getFirstItem)((0, $8Gj3P$getChildNodes)(closestRow, this.collection))?.key;
    }
    getKeyAbove(key) {
        // Expected key is the currently focused cell so we need the parent row key
        let parentRowKey = this.collection.getItem(key).parentKey;
        let layoutInfo = this.getLayoutInfo(parentRowKey);
        let rect = new (0, $8Gj3P$Rect)(layoutInfo.rect.x, 0, layoutInfo.rect.width, layoutInfo.rect.y - 1);
        let closestRow = this.collection.getItem(this._findClosest(layoutInfo.rect, rect)?.key);
        return (0, $8Gj3P$getFirstItem)((0, $8Gj3P$getChildNodes)(closestRow, this.collection))?.key;
    }
    getKeyRightOf(key) {
        ...
        while(key != null){
            ...
            if (item.type === 'item') return (0, $8Gj3P$getFirstItem)((0, $8Gj3P$getChildNodes)(item, this.collection))?.key;
    ...
    getFirstKey() {
        ...
        return firstRow ? (0, $8Gj3P$getFirstItem)((0, $8Gj3P$getChildNodes)(firstRow, this.collection))?.key : null;
    }
    getLastKey() {
        let lastRow = this.collection.getItem(this.collection.getLastKey());
        return lastRow ? (0, $8Gj3P$getFirstItem)((0, $8Gj3P$getChildNodes)(lastRow, this.collection))?.key : null;
    }
    ...
    getKeyPageAbove(key) {
        ...
        if (layoutInfo) {
            ...
            if (layoutInfo && layoutInfo.rect.y > pageY) while(layoutInfo && layoutInfo.rect.y > pageY){
                let childKey = (0, $8Gj3P$getFirstItem)((0, $8Gj3P$getChildNodes)(this.collection.getItem(layoutInfo.key), this.collection))?.key;
    ...
    getKeyForSearch(search, fromKey) {
        ...
        while(key != null){
            ...
            if (item.textValue) {
                ...
                if (this.collator.compare(substring, search) === 0) return (0, $8Gj3P$getFirstItem)((0, $8Gj3P$getChildNodes)(item, this.collection))?.key;
            }
...
export {$47544ce7688f5862$export$64943d2e59d72a29 as BaseLayout};
