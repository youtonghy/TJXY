...
var handleTab = (editor, name, parentListTypes) => {
  ...
  if ((0, import_core5.isNodeActive)(state, name)) return false;
  const previous = (0, import_core5.getPreviousBlockSibling)($from);
  ...
  if (!lastItem.canReplace(lastItem.childCount, lastItem.childCount, import_model.Fragment.from(block))) {
    return false;
  }
  ...
  return editor.commands.command(({ tr, dispatch }) => {
    if (dispatch) {
      tr.delete(blockStart, blockEnd).insert(insideLastItemEnd, import_model.Fragment.from(block));
...
var listItemHasSubList = (typeOrName, state, node) => {
  ...
  const nodeType = (0, import_core6.getNodeType)(typeOrName, state.schema);
...
// src/keymap/list-keymap.ts
var ListKeymap = import_core7.Extension.create({
  name: "listKeymap",
  addOptions() {
    return {
      listTypes: [
