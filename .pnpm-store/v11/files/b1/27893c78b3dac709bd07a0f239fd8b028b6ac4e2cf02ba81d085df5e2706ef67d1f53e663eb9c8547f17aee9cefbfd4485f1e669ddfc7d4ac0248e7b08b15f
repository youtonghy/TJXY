...
import {getFirstItem as $5a8Wo$getFirstItem, getChildNodes as $5a8Wo$getChildNodes} from "react-stately/private/collections/getChildNodes";
import {Size as $5a8Wo$Size, Rect as $5a8Wo$Rect, LayoutInfo as $5a8Wo$LayoutInfo} from "react-stately/useVirtualizerState";
...
const $f44c90ff0e416521$var$DEFAULT_OPTIONS = {
    S: {
        itemPadding: 20,
        minItemSize: {
    ...
    L: {
        ...
        maxItemSize: new (0, $5a8Wo$Size)(Infinity, Infinity),
        margin: 24,
        minSpace: new (0, $5a8Wo$Size)(18, 18),
        maxColumns: Infinity,
        dropSpacing: 100
    }
};
class $f44c90ff0e416521$export$7d2b12578154a735 extends (0, $47544ce7688f5862$export$64943d2e59d72a29) {
    constructor(options = {}){
        super(options);
        let cardSize = 'L';
        this.cardOrientation = options.cardOrientation || 'vertical';
        this.minItemSize = options.minItemSize || $f44c90ff0e416521$var$DEFAULT_OPTIONS[cardSize].minItemSize[this.cardOrientation];
        this.maxItemSize = options.maxItemSize || $f44c90ff0e416521$var$DEFAULT_OPTIONS[cardSize].maxItemSize;
        this.margin = options.margin != null ? options.margin : $f44c90ff0e416521$var$DEFAULT_OPTIONS[cardSize].margin;
        this.minSpace = options.minSpace || $f44c90ff0e416521$var$DEFAULT_OPTIONS[cardSize].minSpace;
        this.maxColumns = options.maxColumns || $f44c90ff0e416521$var$DEFAULT_OPTIONS[cardSize].maxColumns;
        this.itemPadding = options.itemPadding != null ? options.itemPadding : $f44c90ff0e416521$var$DEFAULT_OPTIONS[cardSize].itemPadding[this.cardOrientation][this.scale];
        this.itemSize = null;
        this.numColumns = 0;
        this.numRows = 0;
    ...
    buildCollection() {
        ...
        this.itemSize = new (0, $5a8Wo$Size)(itemWidth, itemHeight);
        ...
        if (this.isLoading) {
            ...
            let rect = new (0, $5a8Wo$Rect)(0, loaderY, visibleWidth, loaderHeight);
            let loader = new (0, $5a8Wo$LayoutInfo)('loader', 'loader', rect);
            this.layoutInfos.set('loader', loader);
            y = loader.rect.maxY;
        }
        if (this.collection.size === 0 && !this.isLoading) {
            let rect = new (0, $5a8Wo$Rect)(0, 0, visibleWidth, visibleHeight);
            let placeholder = new (0, $5a8Wo$LayoutInfo)('placeholder', 'placeholder', rect);
            this.layoutInfos.set('placeholder', placeholder);
            y = placeholder.rect.maxY;
        }
        this.contentSize = new (0, $5a8Wo$Size)(visibleWidth, y);
    }
    buildChild(node, y, index) {
        ...
        let rect = new (0, $5a8Wo$Rect)(x, y, this.itemSize.width, this.itemSize.height);
        // TODO: Perhaps have it so that the child key for each row is stored with the layoutInfo?
        let layoutInfo = new (0, $5a8Wo$LayoutInfo)(node.type, node.key, rect);
    ...
    getKeyBelow(key) {
        ...
        return row ? (0, $5a8Wo$getFirstItem)((0, $5a8Wo$getChildNodes)(row, this.collection))?.key : null;
    }
    getKeyAbove(key) {
        // Expected key is the currently focused cell so we need the parent row key
        let parentRowKey = this.collection.getItem(key).parentKey;
...
export {$f44c90ff0e416521$export$7d2b12578154a735 as GridLayout};
