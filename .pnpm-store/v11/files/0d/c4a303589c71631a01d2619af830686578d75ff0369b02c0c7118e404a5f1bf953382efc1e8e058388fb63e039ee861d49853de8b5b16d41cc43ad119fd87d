...
// src/keymap/listHelpers/handleTab.ts
...
var handleTab = (editor, name, parentListTypes) => {
  ...
  if ((0, import_core8.isNodeActive)(state, name)) return false;
  const previous = (0, import_core8.getPreviousBlockSibling)($from);
  ...
  if (!lastItem.canReplace(lastItem.childCount, lastItem.childCount, import_model2.Fragment.from(block))) {
    return false;
  }
  ...
  return editor.commands.command(({ tr, dispatch }) => {
    if (dispatch) {
      tr.delete(blockStart, blockEnd).insert(insideLastItemEnd, import_model2.Fragment.from(block));
      tr.setSelection(import_state.TextSelection.create(tr.doc, insideLastItemEnd + 1));
      tr.scrollIntoView();
    }
    return true;
  });
...
var listItemHasSubList = (typeOrName, state, node) => {
  ...
  const nodeType = (0, import_core9.getNodeType)(typeOrName, state.schema);
...
// src/keymap/list-keymap.ts
var ListKeymap = import_core10.Extension.create({
  name: "listKeymap",
  addOptions() {
    return {
      listTypes: [
...
var OrderedList = import_core11.Node.create({
  name: "orderedList",
  addOptions() {
    return {
      itemTypeName: "listItem",
  ...
  renderHTML({ HTMLAttributes }) {
    ...
    const attrs = (0, import_core11.mergeAttributes)(this.options.HTMLAttributes, attributesWithoutType);
    if (start !== 1) {
      attrs.start = start;
    }
    if (type && type !== "1") {
  ...
  addInputRules() {
    ...
    let inputRule = (0, import_core11.wrappingInputRule)({
    ...
    if (this.options.keepMarks || this.options.keepAttributes) {
      inputRule = (0, import_core11.wrappingInputRule)({
        find: orderedListInputRegex,
        type: this.type,
        keepMarks: this.options.keepMarks,
        keepAttributes: this.options.keepAttributes,
...
var TaskItem = import_core12.Node.create({
  name: "taskItem",
  addOptions() {
    return {
      nested: false,
  ...
  renderHTML({ node, HTMLAttributes }) {
    return [
      "li",
      (0, import_core12.mergeAttributes)(this.options.HTMLAttributes, HTMLAttributes, {
        "data-type": this.name
      }),
      [
        "label",
  ...
  renderMarkdown: (node, h) => {
    ...
    return (0, import_core12.renderNestedMarkdownContent)(node, h, prefix);
  },
  addExtensions() {
    ...
    return ({ node, HTMLAttributes, getPos, editor }) => {
      ...
      return {
        ...
        update: (updatedNode) => {
          ...
          const newHTMLAttributes = (0, import_core12.getRenderedAttributes)(updatedNode, extensionAttributes);
  ...
  addInputRules() {
    return [
      (0, import_core12.wrappingInputRule)({
        find: inputRegex,
        type: this.type,
        getAttributes: (match) => ({
...
var TaskList = import_core13.Node.create({
  name: "taskList",
  addOptions() {
    return {
  ...
  renderHTML({ HTMLAttributes }) {
    return [
      "ul",
      (0, import_core13.mergeAttributes)(this.options.HTMLAttributes, HTMLAttributes, { "data-type": this.name }),
      0
    ];
  },
  parseMarkdown: (token, h) => {
  ...
  markdownTokenizer: {
    ...
    tokenize(src, tokens, lexer) {
      const parseTaskListContent = (content) => {
        const nestedResult = (0, import_core13.parseIndentedBlocks)(
          content,
          {
      ...
      const result = (0, import_core13.parseIndentedBlocks)(
        src,
        {
          itemPattern: /^(\s*)([-+*])\s+\[([ xX])\]\s+(.*)$/,
          extractItemData: (match) => ({
...
// src/kit/index.ts
var ListKit = import_core14.Extension.create({
  name: "listKit",
  addExtensions() {
    const extensions = [];
