...
| React Admin cloud Storage | ✅ | ⚠️ 管理员可选择目标库、启动服务端 OAuth、确认回调、选择 My Drive 或分页 Shared Drive、逐层选择 Google/OneDrive Personal 目录并提交绑定；Storage 状态/重授权、metadata、迁移及冲突管理页面未实现 |
...
| Google Drive 原生 backend | My Drive + Shared Drive | ⚠️ Files/Changes/Shared Drive/OAuth refresh/严格 Range/429 Retry-After、服务端 authorization-code + S256 PKCE、一次性 session-bound state、Google `about.user` 账号身份、分页 Shared Drive 枚举、My Drive/Shared Drive 目录选择、版本化 AEAD credential loader、同进程 runtime 热激活、Admin 向导及管理员初始绑定/cursor 已实现；在线重新授权/轮换命令未实现 |
| OneDrive 原生 backend | **仅 Personal** | ⚠️ Files/children/Delta、OAuth refresh-token 加密轮换、严格 Range、bearer-safe 临时 URL、runtime loader/worker 与同进程热激活、服务端 authorization-code + S256 PKCE、一次性 session-bound state、Microsoft Graph 身份/Personal drive/root 推导、目录选择、Admin 向导及管理员初始绑定/root/delta cursor 已实现；在线重新授权/轮换命令未实现 |
