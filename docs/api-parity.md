...
| 8.1 | 相似推荐 | `GET /Items/{id}/Similar` | 仅真实可见馆藏与当前用户状态 | ✅ Movie/Series 同类型、标签或人物强相关门槛、语言/制作公司/国家/年份加权、已看排除、收藏未看加权、稳定排序及最多 20 项响应已实现；详情页请求 4 项并以原生横向轨道展示，空结果不随机补齐 |
...
| Search | P1.5 + Redis | ✅ `GET /Search/Hints` 与 `GET /Items?searchTerm=...` 已认证、按可见性筛选、把 SQL 通配符作为普通文本、支持类型筛选和稳定分页；`/Items` 搜索不会隐式开启递归，缓存按完整查询维度与 catalog/user revision 隔离 |
