...
export function SearchPage() {
  ...
  return (
    <div className="space-y-6">
      <div><h1 className="text-3xl font-semibold">Search</h1><p className="mt-1 text-muted">Find something to watch.</p></div>
      <SearchField aria-label="Search media" value={value} onChange={(next) => { setValue(next); setParams(next ? { q: next } : {}); }}>
        <SearchField.Group><SearchField.SearchIcon /><SearchField.Input placeholder="Search movies, series, and episodes" /></SearchField.Group>
      </SearchField>
      {loading ? (
        ...
        <p className="py-12 text-center text-muted">No results for “{query}”.</p>
      ) : (
        <MediaRow items={popular} limitToTwoRows moreTo="/app/rankings" title="Popular recommendations" />
      )}
    </div>
  );
}
