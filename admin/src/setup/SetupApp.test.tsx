...
beforeEach(() => {
  ...
  networkMock.mockReset().mockResolvedValue({
    ...
    port: 8096,
    publicUrl: null,
    destinationUrl: 'http://127.0.0.1:8096/login?redirect=%2Fadmin',
  });
  brandingMock.mockReset().mockResolvedValue(`/Branding/Assets/logo-${'a'.repeat(64)}.png`);
  completeMock.mockReset().mockResolvedValue('http://127.0.0.1:8096/login?redirect=%2Fadmin');
  recoverMock.mockReset().mockResolvedValue('http://127.0.0.1:8096/login?redirect=%2Fadmin');
});
...
it('requires a successful network preflight for the unchanged draft', async () => {
  ...
  expect(screen.getByRole('button', { name: 'Continue' })).toBeDisabled();
  await user.click(screen.getByRole('button', { name: 'Validate network' }));
  expect(await screen.findByText('http://127.0.0.1:8096/login?redirect=%2Fadmin')).toBeVisible();
