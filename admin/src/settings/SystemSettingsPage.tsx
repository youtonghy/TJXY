    ...
    <div className="space-y-5">
      ...
      <Card>
        <Card.Header className="items-start gap-3">
          ...
          <div>
            <Card.Title>{tr('admin.system.mediaBrowser')}</Card.Title>
            <Card.Description>{tr('admin.system.mediaBrowserDescription')}</Card.Description>
          </div>
        </Card.Header>
        <Card.Content className="space-y-3">
            ...
            <div className="flex items-end gap-2" key={`media-browser-root-${String(index)}`}>
              ...
              >
                ...
                <Input
                  ...
                  value={root}
                  onChange={(event) => {
                    const next = [...settings.mediaBrowserRoots];
              ...
              <Button
                ...
                isIconOnly
                onPress={() => {
                  patch('mediaBrowserRoots', settings.mediaBrowserRoots.filter((_, itemIndex) => itemIndex !== index));
                }}
                variant="ghost"
              >
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 border-t border-border pt-4">
            ...
            <Button
              aria-label={tr('admin.system.addMediaBrowserRoot')}
              isDisabled={settings.environmentOverrides.mediaBrowserRoots}
              onPress={() => { patch('mediaBrowserRoots', [...settings.mediaBrowserRoots, '']); }}
              size="sm"
              variant="secondary"
            >
              <Plus aria-hidden="true" className="size-4" />
              {tr('admin.system.addMediaBrowserRoot')}
            </Button>
          </div>
          {settings.environmentOverrides.mediaBrowserRoots && (
            <p className="text-xs text-warning">{tr('admin.system.environmentOverride')}</p>
          )}
        </Card.Content>
      </Card>
